 ...
 */
import { ALLOWED_TRANSITIONS } from './pathway';
import { recordAudit, notifyVisit } from '../common/audit';
...
const UPDATE_SQL: Record<StepStatus, string> = {
  pending: 'UPDATE visit_steps SET status = $1 WHERE id = $2',
...
export async function changeStatus(
  client: Executor,
  { stepId, target, actorUserId = null, actorKind = 'user', reason = null }: ChangeStatusInput,
): Promise<LockedStep> {
  const { rows } = await client.query<LockedStep>(
...
async function completeIfFinished(client: Executor, visitId: number): Promise<boolean> {
