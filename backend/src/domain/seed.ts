import { csvBool, csvNumberOrNull, csvOrNull, type CsvRow } from '../common/csv';
import { hashPassword } from '../modules/auth/auth.service';
import { qrToken } from '../common/ids';
import type { Executor } from '../db/db.service';

export type SeedFiles = Record<string, CsvRow[]>;
 ...
 */
export async function importAll(client: Executor, files: SeedFiles): Promise<SeedSummary> {
...
async function importFloors(client: Executor, nodeRows: CsvRow[]): Promise<Map<string, number>> {
...
async function importDepartments(client: Executor, rows: CsvRow[], floors: Map<string, number>): Promise<Map<string, number>> {
...
async function importNodes(client: Executor, rows: CsvRow[], floors: Map<string, number>, departments: Map<string, number>): Promise<Map<string, number>> {
...
async function linkDepartmentEntrances(client: Executor, rows: CsvRow[], nodes: Map<string, number>): Promise<void> {
...
async function importEdges(client: Executor, rows: CsvRow[], nodes: Map<string, number>): Promise<number> {
  const OPPOSITE_TURN: Record<string, string> = { left: 'right', right: 'left', straight: 'straight', back: 'back' };
  let count = 0;
  ...
  for (const row of rows) {
    ...
    const directions: {
      from: string; to: string; turn: string | null;
      instructionTh: string | null; instructionEn: string | null;
    }[] = [
      {
        from: row.from,
        to: row.to,
...
async function importPathways(client: Executor, templateRows: CsvRow[], rows: CsvRow[], nodes: Map<string, number>): Promise<number> {
  const templates = new Map<string, number>();
  const stepIds = new Map<string, number>();   // 'CODE#sequence' → id
...
async function importUsers(client: Executor, rows: CsvRow[], departments: Map<string, number>): Promise<number> {
  for (const row of rows) {
    await client.query(
...
async function importPatients(client: Executor, rows: CsvRow[]): Promise<number> {
  for (const row of rows) {
    await client.query(
 ...
 */
async function importNodePhotos(client: Executor, rows: CsvRow[], nodes: Map<string, number>): Promise<number> {
  for (const row of rows) {
