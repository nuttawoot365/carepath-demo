...
import { costFor, dijkstra, shortestTimes } from './graph';
import { buildInstructions, type Instruction } from './instructions';
import { buildLegs, type Leg } from './legs';
import { planNextStep, topologicalOrder, type Warning } from './pathway';
import { recordAudit } from '../common/audit';
import { ClockService, endOfToday } from '../common/clock.service';
import { shortCode, visitToken } from '../common/ids';
import { conflict, notFound } from '../common/errors';
import type { PatientProfile } from '../common/validate';
import type { StepRow, VisitRow } from './types';
...
export class VisitService {
  ...
  async findBy(field: 'id' | 'token' | 'short_code', value: string | number): Promise<VisitRow> {
    ...
    const { rows } = await this.db.query<VisitRow>(`${VISIT_SQL} WHERE ${column} = $1`, [value]);
    if (rows.length === 0) throw notFound('ไม่พบบัตรคิวใบนี้', 'No such visit');
    return rows[0];
  }

  async loadSteps(visitId: number): Promise<StepRow[]> {
    const { rows } = await this.db.query<StepRow>(STEP_SQL, [visitId]);
    return rows.map((row) => ({ ...row, sequence: Number(row.sequence), requires: row.requires.map(Number) }));
  }

  /** เปิด visit ใหม่จากแม่แบบ — visits + visit_steps + prereqs + audit ในทรานแซกชันเดียว */
  async create(client: Executor, input: CreateVisitInput) {
    ...
    const idByTemplateStep = new Map<number, number>();
    ...
    for (const step of templateSteps) {
      for (const requiredId of step.requires as number[]) {
        await client.query(
    ...
    return visit as { id: number; token: string; short_code: string; ticket_no: string };
  }

  /** แทรกขั้นใหม่ให้ visit ที่กำลังเดินอยู่ · ลำดับใหม่มาจาก topological sort */
  async insertStep(client: Executor, visit: { id: number }, input: InsertStepInput): Promise<number> {
    ...
    const all = [
      ...existing.map((step) => ({
        ...
        requires: (step.requires as number[]).map(Number),
      })),
    ...
    await recordAudit(client, {
      actorUserId: input.addedBy ?? null,
   ...
   */
  async planFor(visit: VisitRow, steps: StepRow[]): Promise<Plan> {
    const graph = await this.graphs.getGraph();
    const profile: PatientProfile = { ...(visit.profile ?? {}), has_phone: visit.has_phone };
    const cost = costFor(profile);
    const fromId = visit.current_node_id;

    const walkSeconds = fromId ? shortestTimes(graph, fromId, cost) : new Map<number, number>();
    ...
    const { next, ready, warnings } = planNextStep(steps, byPoint, this.clock.minutesOfDay());
  ...
  async state(visit: VisitRow, steps: StepRow[]) {
    const { next, ready, warnings, route } = await this.planFor(visit, steps);

    const { rows: notifications } = await this.db.query(
  ...
  /** อ่าน visit ใหม่จากฐานข้อมูลแล้วคืนสถานะทั้งชุด — ใช้ปิดท้ายทุก endpoint ที่เปลี่ยนข้อมูล */
  async stateOf(visitId: number) {
    const visit = await this.findBy('id', visitId);
    return this.state(visit, await this.loadSteps(visitId));
  }
}

export const publicVisit = (visit: VisitRow) => ({
  id: visit.id,
...
export const publicStep = (step: StepRow) => ({
  id: step.id,
  sequence: Number(step.sequence),
...
export async function nodeIdByCode(executor: Executor, code: string): Promise<number> {
...
async function nextTicketNo(client: Executor): Promise<string> {
  const { rows } = await client.query(
